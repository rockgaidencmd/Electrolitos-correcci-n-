// ===== activacion.js · MS-Electrolitos · Un código = un dispositivo =====
// Comparte los códigos con MEDISHORT360 (misma colección 'codigos_x3').
...
const COLECCION   = 'codigos_x3';            // misma colección que MediShort360
const LS_KEY      = 'mselectro_activado';
const LS_CODE_KEY = 'mselectro_codigo';

const app = initializeApp(firebaseConfig, 'mselectro-' + COLECCION);
...
async function intentarActivar() {
  ...
  if (errMsg) { errMsg.textContent = ''; errMsg.classList.remove('visible'); }
...
function mostrarError(msg) {
  const errMsg = document.getElementById('gate-error');
  const input  = document.getElementById('gate-input');
  if (errMsg) { errMsg.textContent = msg; errMsg.classList.add('visible'); }
  if (input) { input.classList.remove('shake'); void input.offsetWidth; input.classList.add('shake'); }
}

document.addEventListener('DOMContentLoaded', () => {
  const gate = document.getElementById('activation-gate');
  if (yaActivado()) { if (gate) gate.remove(); return; }
